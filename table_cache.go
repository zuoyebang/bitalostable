...
var tableCacheLabels = pprof.Labels("bitalostable", "table-cache")
...
func newTableCacheContainer(
	...
	if tc != nil {
		if tc.cache != opts.Cache {
			panic("bitalostable: underlying cache for the table cache and db are different")
		}
		tc.Ref()
	} else {
...
func NewTableCache(cache *Cache, numShards int, size int) *TableCache {
	if size == 0 {
		panic("bitalostable: cannot create a table cache of size 0")
	} else if numShards == 0 {
		panic("bitalostable: cannot create a table cache with 0 shards")
	}
