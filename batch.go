...
import (
	...
	"github.com/zuoyebang/bitalostable/internal/batchskl"
	...
	"github.com/zuoyebang/bitalostable/internal/rangedel"
	"github.com/zuoyebang/bitalostable/internal/rangekey"
	"github.com/zuoyebang/bitalostable/internal/rawalloc"
)
...
var ErrNotIndexed = errors.New("bitalostable: batch not indexed")

// ErrInvalidBatch indicates that a batch is invalid or otherwise corrupted.
var ErrInvalidBatch = errors.New("bitalostable: invalid batch")

// ErrBatchTooLarge indicates that a batch is invalid or otherwise corrupted.
var ErrBatchTooLarge = errors.Newf("bitalostable: batch too large: >= %s", humanize.Uint64(maxBatchSize))
...
// copying into an intermediary buffer then having bitalostable.Batch copy off of it.
...
func newFlushBatch(db *DB, n int) *Batch {
	b := batchPool.Get().(*Batch)
	b.db = db
	b.isFlush = true
	b.alloc = manual.New(n)
	b.data = b.alloc[:batchHeaderLen]
	b.setCount(0)
	b.setSeqNum(0)
	return b
}
...
func (b *Batch) Apply(batch *Batch, _ *WriteOptions) error {
	...
	if len(batch.data) < batchHeaderLen {
		return base.CorruptionErrorf("bitalostable: invalid batch")
	}
...
func (b *Batch) Reset() {
	...
	if b.data != nil {
		if cap(b.data) > batchMaxRetainedSize || b.isFlush {
...
func newFlushableBatch(batch *Batch, comparer *Comparer) *flushableBatch {
	b := &flushableBatch{
		...
		offsetsArena: arena.NewArena(),
	}
	b.offsets = arena.MakeSlice[flushableBatchEntry](b.offsetsArena, 0, int(batch.Count()))
	if b.data != nil {
...
func (b *flushableBatch) setSeqNum(seqNum uint64) {
	if b.seqNum != 0 {
		panic(fmt.Sprintf("bitalostable: flushableBatch.seqNum already set: %d", b.seqNum))
	}
	b.seqNum = seqNum
...
// exposing this method in the public bitalostable interface.
func batchSort(
	i interface{},
) (
