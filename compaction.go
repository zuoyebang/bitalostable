// Copyright 2013 The LevelDB-Go and Pebble and Bitalostored Authors. All rights reserved. Use
...
var errEmptyTable = errors.New("bitalostable: empty table")
var errFlushInvariant = errors.New("bitalostable: flush next log number is unset")

var compactLabels = pprof.Labels("bitalostable", "compact")
var flushLabels = pprof.Labels("bitalostable", "flush")
var gcLabels = pprof.Labels("bitalostable", "gc")
...
func (c *compaction) errorOnUserKeyOverlap(ve *versionEdit) error {
	if n := len(ve.NewFiles); n > 1 {
		...
		if !prevMeta.Largest.IsExclusiveSentinel() &&
			...
			return errors.Errorf("bitalostable: compaction split user key across two sstables: %s in %s and %s",
...
func (d *DB) flush(needReport bool) {
	pprof.Do(context.Background(), flushLabels, func(context.Context) {
		...
		var err error
		if bytesFlushed, err = d.flush1(needReport); err != nil {
...
func (d *DB) flush1(needReport bool) (bytesFlushed uint64, err error) {
	var n int
	for ; n < len(d.mu.mem.queue)-1; n++ {
		if !d.mu.mem.queue[n].readyForFlush() {
			break
...
func (h *deleteCompactionHint) canDelete(cmp Compare, m *fileMetadata, snapshots []uint64) bool {
	...
	case deleteCompactionHintTypePointAndRangeKey:
		...
		// care of this.
	default:
		panic(fmt.Sprintf("bitalostable: unknown delete compaction hint type: %d", h.hintType))
	}
...
func (d *DB) runCompaction(
	...
	defer func() {
		if ve != nil {
			...
			if err != nil {
				d.opts.Logger.Fatalf("bitalostable: version edit validation failed: %s", err)
			}
		}
	}()
	...
	finishOutput := func(splitKey []byte) error {
			...
			if writerMeta.SmallestRangeDel.UserKey != nil {
				...
				if c < 0 {
					return errors.Errorf(
						"bitalostable: smallest range tombstone start key is less than previous sstable largest key: %s < %s",
					...
					// partitioned.
					return errors.Errorf(
						"bitalostable: invariant violation: previous sstable largest key %s, current sstable smallest rangedel: %s",
						prevMeta.Largest.Pretty(d.opts.Comparer.FormatKey),
						writerMeta.SmallestRangeDel.Pretty(d.opts.Comparer.FormatKey),
					)
				}
		...
		if splitKey != nil && writerMeta.LargestRangeDel.UserKey != nil &&
			...
			return errors.Errorf(
				"bitalostable: invariant violation: rangedel largest key %q extends beyond split key %q",
		...
		if c.smallest.UserKey != nil {
			...
			case v >= 0:
				// Nothing to do.
			case v < 0:
				return errors.Errorf("bitalostable: compaction output grew beyond bounds of input: %s < %s",
	...
	for key, val := iter.First(); key != nil || !c.rangeDelFrag.Empty() || !c.rangeKeyFrag.Empty(); {
		...
		for ; key != nil; key, val = iter.Next() {
			...
			if key.Kind() == InternalKeyKindSet {
				if d.opts.KvCheckExpireFunc(key.UserKey, val) {
...
func (d *DB) enableFileDeletions() {
	if d.mu.cleaner.disabled <= 0 || d.mu.cleaner.cleaning {
		panic("bitalostable: file deletion disablement invariant violated")
	}
	d.mu.cleaner.disabled--
