...
func (f *Fragmenter) checkInvariants(buf []Span) {
	for i := 1; i < len(buf); i++ {
		if f.Cmp(buf[i].Start, buf[i].End) >= 0 {
			panic(fmt.Sprintf("bitalostable: empty pending span invariant violated: %s", buf[i]))
		}
		if f.Cmp(buf[i-1].Start, buf[i].Start) != 0 {
			panic(fmt.Sprintf("bitalostable: pending span invariant violated: %s %s",
...
func (f *Fragmenter) Add(s Span) {
	if f.finished {
		panic("bitalostable: span fragmenter already finished")
	} else if s.KeysOrder != ByTrailerDesc {
		panic("bitalostable: span keys unexpectedly not in trailer descending order")
	}
	if f.flushedKey != nil {
		switch c := f.Cmp(s.Start, f.flushedKey); {
		case c < 0:
			panic(fmt.Sprintf("bitalostable: start key (%s) < flushed key (%s)",
	...
	if len(f.pending) > 0 {
		...
		case c > 0:
			panic(fmt.Sprintf("bitalostable: keys must be added in order: %s > %s",
...
func (f *Fragmenter) Covers(key base.InternalKey, snapshot uint64) bool {
	...
	if f.Cmp(f.pending[0].Start, key.UserKey) > 0 {
		panic(fmt.Sprintf("bitalostable: keys must be in order: %s > %s",
