...
package keyspan // import "github.com/zuoyebang/bitalostable/internal/keyspan"
...
func (s *Span) SmallestKey() base.InternalKey {
	if len(s.Keys) == 0 {
		panic("bitalostable: Span contains no keys")
	} else if s.KeysOrder != ByTrailerDesc {
		panic("bitalostable: span's keys unexpectedly not in trailer order")
	}
