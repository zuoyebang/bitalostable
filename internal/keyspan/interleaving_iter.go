...
func (i *InterleavingIter) interleaveForward(
	...
	for {
		...
		if invariants.Enabled {
			...
			if i.pointKeyInterleaved {
				panic("bitalostable: invariant violation: point key interleaved")
			}
			switch {
			case i.span == nil:
			case i.pointKey == nil:
			default:
				...
				if i.keyspanInterleaved && i.cmp(i.pointKey.UserKey, i.span.End) >= 0 {
					panic("bitalostable: invariant violation: span interleaved, but point key >= span end")
				}
			}
		}
		...
		case i.pointKey == nil:
			if i.pointKeyInterleaved {
				panic("bitalostable: invariant violation: point key already interleaved")
			}
...
func (i *InterleavingIter) verify(k *base.InternalKey, v []byte) (*base.InternalKey, []byte) {
	...
	if invariants.Enabled {
		...
		case k != nil && !i.keyspanInterleaved && !i.pointKeyInterleaved:
			panic("bitalostable: invariant violation: both keys marked as noninterleaved")
		case i.dir == -1 && k != nil && i.keyspanInterleaved == i.pointKeyInterleaved:
			...
			panic(fmt.Sprintf("bitalostable: invariant violation: interleaving (point %t, span %t)",
				i.pointKeyInterleaved, i.keyspanInterleaved))
		case i.dir == -1 && i.spanMarkerTruncated:
			panic("bitalostable: invariant violation: truncated span key in reverse iteration")
		case k != nil && i.lower != nil && i.cmp(k.UserKey, i.lower) < 0:
			panic("bitalostable: invariant violation: key < lower bound")
		case k != nil && i.upper != nil && i.cmp(k.UserKey, i.upper) >= 0:
			panic("bitalostable: invariant violation: key ≥ upper bound")
		case i.span != nil && k != nil && i.mask != nil && i.pointKeyInterleaved &&
			...
			panic("bitalostable: invariant violation: point key eligible for skipping returned")
		}
	}
	return k, v
}
