...
// optimization bitalostable.mergingIter performs for RANGEDELs during point key
	...
	if invariants.Enabled {
		for i := range m.levels {
			...
			if l.heapKey.kind != boundKindInvalid && m.cmp(l.heapKey.key, m.start) > 0 {
				panic("bitalostable: invariant violation: max-heap key > m.start")
			}
		}
	}
	...
	if invariants.Enabled {
		for i := range m.levels {
			...
			if l.heapKey.kind != boundKindInvalid && m.cmp(l.heapKey.key, m.end) < 0 {
				panic("bitalostable: invariant violation: min-heap key < m.end")
			}
		}
	}
...
func (m *MergingIter) synthesizeKeys(dir int8) (bool, *Span) {
	if invariants.Enabled {
		if m.cmp(m.start, m.end) >= 0 {
			panic(fmt.Sprintf("bitalostable: invariant violation: span start ≥ end: %s >= %s", m.start, m.end))
		}
	}
...
// mergingIterHeap is copied from mergingIterHeap defined in the root bitalostable
