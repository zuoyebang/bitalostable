	...
	for i := range keys {
		...
		case base.InternalKeyKindRangeKeySet:
			if invariants.Enabled && len(dst.Keys) > 0 && cmp(dst.Keys[len(dst.Keys)-1].Suffix, keys[i].Suffix) > 0 {
				panic("bitalostable: keys unexpectedly not in ascending suffix order")
			}
		...
		case base.InternalKeyKindRangeKeyDelete:
			...
			continue
		default:
			return base.CorruptionErrorf("bitalostable: unrecognized range key kind %s", keys[i].Kind())
		}
	}
	...
	for i := range a.Keys {
		if invariants.Enabled {
			if a.Keys[i].Kind() != base.InternalKeyKindRangeKeySet ||
				...
				panic("bitalostable: unexpected non-RangeKeySet during defragmentation")
			}
			if i > 0 && (ui.comparer.Compare(a.Keys[i].Suffix, a.Keys[i-1].Suffix) < 0 ||
				ui.comparer.Compare(b.Keys[i].Suffix, b.Keys[i-1].Suffix) < 0) {
				panic("bitalostable: range keys not ordered by suffix during defragmentation")
			}
		}
...
func coalesce(keysBySuffix *keysBySuffix, keys []keyspan.Key, dst *[]keyspan.Key) error {
	...
	for i := 0; i < len(keys) && !deleted; i++ {
		...
		if invariants.Enabled && i > 0 && k.Trailer > keys[i-1].Trailer {
			panic("bitalostable: invariant violation: span keys unordered")
		}
		...
		case base.InternalKeyKindRangeKeyDelete:
			...
			deleted = true
		default:
			return base.CorruptionErrorf("bitalostable: unexpected range key kind %s", k.Kind())
		}
	}
