...
func (e *Encoder) Encode(s *keyspan.Span) error {
	...
	for i := range s.Keys {
		...
		case base.InternalKeyKindRangeKeyDelete:
			del = true
		default:
			return base.CorruptionErrorf("bitalostable: %s key kind is not a range key", s.Keys[i].Kind())
		}
	}
...
func Decode(ik base.InternalKey, v []byte, keysDst []keyspan.Key) (keyspan.Span, error) {
	...
	if !ok {
		return keyspan.Span{}, base.CorruptionErrorf("bitalostable: unable to decode range key end from %s", ik.Kind())
	}
	s.Keys = keysDst
	...
	case base.InternalKeyKindRangeKeySet:
		for len(v) > 0 {
			...
			if !ok {
				return keyspan.Span{}, base.CorruptionErrorf("bitalostable: unable to decode range key suffix-value tuple")
			}
	...
	case base.InternalKeyKindRangeKeyUnset:
		for len(v) > 0 {
			...
			if !ok {
				return keyspan.Span{}, base.CorruptionErrorf("bitalostable: unable to decode range key unset suffix")
			}
			s.Keys = append(s.Keys, keyspan.Key{
				Trailer: ik.Trailer,
				Suffix:  suffix,
	...
	case base.InternalKeyKindRangeKeyDelete:
		if len(v) > 0 {
			return keyspan.Span{}, base.CorruptionErrorf("bitalostable: RANGEKEYDELs must not contain additional data")
		}
		s.Keys = append(s.Keys, keyspan.Key{Trailer: ik.Trailer})
	default:
		return keyspan.Span{}, base.CorruptionErrorf("bitalostable: %s is not a range key", ik.Kind())
	}
	return s, nil
}
