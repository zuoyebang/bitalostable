...
func (o *initOp) run(t *test, h *history) {
	t.batches = make([]*bitalostable.Batch, o.batchSlots)
	t.iters = make([]*retryableIter, o.iterSlots)
	t.snapshots = make([]*bitalostable.Snapshot, o.snapshotSlots)
	h.Recordf("%s", o)
}
...
func (o *ingestOp) build(t *test, h *history, b *bitalostable.Batch, i int) (string, error) {
...
func (o *ingestOp) collapseBatch(
	...
	if pointIter != nil {
		...
		for key, value := pointIter.First(); key != nil; key, value = pointIter.Next() {
			...
			var err error
			switch key.Kind() {
			case bitalostable.InternalKeyKindDelete:
				err = collapsed.Delete(key.UserKey, nil)
			case bitalostable.InternalKeyKindSingleDelete:
				err = collapsed.SingleDelete(key.UserKey, nil)
			case bitalostable.InternalKeyKindSet:
				err = collapsed.Set(key.UserKey, value, nil)
			case bitalostable.InternalKeyKindMerge:
				err = collapsed.Merge(key.UserKey, value, nil)
			case bitalostable.InternalKeyKindLogData:
...
func (o *newIterUsingCloneOp) run(t *test, h *history) {
	...
	cloneOpts := bitalostable.CloneOptions{
...
func iterOptions(o iterOpts) *bitalostable.IterOptions {
	if o.IsZero() {
		return nil
	}
	var lower, upper []byte
	...
	opts := &bitalostable.IterOptions{
		LowerBound: lower,
		UpperBound: upper,
		KeyTypes:   bitalostable.IterKeyType(o.keyTypes),
		RangeKeyMasking: bitalostable.RangeKeyMasking{
			Suffix: o.maskSuffix,
		},
	}
	if opts.RangeKeyMasking.Suffix != nil {
		opts.RangeKeyMasking.Filter = func() bitalostable.BlockPropertyFilterMask {
			return blockprop.NewMaskingFilter()
		}
	}
	if o.filterMax > 0 {
		opts.PointKeyFilters = []bitalostable.BlockPropertyFilter{
...
func validityStateToStr(validity bitalostable.IterValidityState) (bool, string) {
	// We can't distinguish between IterExhausted and IterAtLimit in a
	// deterministic manner.
	switch validity {
	case bitalostable.IterExhausted, bitalostable.IterAtLimit:
		return false, "invalid"
	case bitalostable.IterValid:
		return true, "valid"
	default:
