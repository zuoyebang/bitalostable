...
import (
	...
	"github.com/zuoyebang/bitalostable/internal/metamorphic"
)

var (
	factor   int
	seed     int64
	versions bitalostableVersions
)
...
type bitalostableVersion struct {
	Label          string
	SHA            string
...
func runCrossVersion(
	ctx context.Context, t *testing.T, versions bitalostableVersions, seed int64, factor int,
) error {
...
func (v bitalostableVersion) String() string {
	return fmt.Sprintf("%s,%s,%s", v.Label, v.SHA, v.TestBinaryPath)
}

type bitalostableVersions []bitalostableVersion

func (f *bitalostableVersions) String() string {
	var buf bytes.Buffer
	for i, v := range *f {
		if i > 0 {
			fmt.Fprint(&buf, " ")
...
func (f *bitalostableVersions) Set(value string) error {
	...
	*f = append(*f, bitalostableVersion{
		Label:          fields[0],
		SHA:            fields[1],
