...
import (
	...
	"github.com/zuoyebang/bitalostable/internal/errorfs"
...
type test struct {
	...
	writeOpts *bitalostable.WriteOptions
	tmpDir    string
	...
	batches   []*bitalostable.Batch
	iters     []*retryableIter
	snapshots []*bitalostable.Snapshot
}
...
func (t *test) init(h *history, dir string, testOpts *testOptions) error {
	t.dir = dir
	t.testOpts = testOpts
	t.writeOpts = bitalostable.NoSync
	if testOpts.strictFS {
		t.writeOpts = bitalostable.Sync
	}
	t.opts = testOpts.opts.EnsureDefaults()
	t.opts.Logger = bitalostable.DefaultLogger
	t.opts.EventListener = bitalostable.MakeLoggingEventListener(t.opts.Logger)
	t.opts.DebugCheck = func(db *bitalostable.DB) error {
		// Wrap the ordinary DebugCheckLevels with retrying
		// of injected errors.
		return withRetries(func() error {
			return bitalostable.DebugCheckLevels(db)
		})
	}
	...
	t.opts.EventListener.CompactionEnd = func(info bitalostable.CompactionInfo) {
		t.opts.Logger.Infof("%s", info)
		maybeExit(info.Err)
	}
	t.opts.EventListener.FlushEnd = func(info bitalostable.FlushInfo) {
		t.opts.Logger.Infof("%s", info)
		if info.Err != nil && !strings.Contains(info.Err.Error(), "bitalostable: empty table") {
			maybeExit(info.Err)
		}
	}
	t.opts.EventListener.ManifestCreated = func(info bitalostable.ManifestCreateInfo) {
		t.opts.Logger.Infof("%s", info)
		maybeExit(info.Err)
	}
	t.opts.EventListener.ManifestDeleted = func(info bitalostable.ManifestDeleteInfo) {
		t.opts.Logger.Infof("%s", info)
		maybeExit(info.Err)
	}
	t.opts.EventListener.TableDeleted = func(info bitalostable.TableDeleteInfo) {
		t.opts.Logger.Infof("%s", info)
		maybeExit(info.Err)
	}
	t.opts.EventListener.TableIngested = func(info bitalostable.TableIngestInfo) {
		t.opts.Logger.Infof("%s", info)
		maybeExit(info.Err)
	}
	t.opts.EventListener.WALCreated = func(info bitalostable.WALCreateInfo) {
		t.opts.Logger.Infof("%s", info)
		maybeExit(info.Err)
	}
	t.opts.EventListener.WALDeleted = func(info bitalostable.WALDeleteInfo) {
		t.opts.Logger.Infof("%s", info)
		maybeExit(info.Err)
	}

	var db *bitalostable.DB
	var err error
	err = withRetries(func() error {
		db, err = bitalostable.Open(dir, t.opts)
		return err
	})
	if err != nil {
		return err
...
func (t *test) restartDB() error {
	...
	err := withRetries(func() (err error) {
		t.db, err = bitalostable.Open(t.dir, t.opts)
		return err
	})
	t.opts.Cache.Unref()
	return err
...
func (t *test) setBatch(id objID, b *bitalostable.Batch) {
	if id.tag() != batchTag {
...
func (t *test) setIter(id objID, i *bitalostable.Iterator, filterMin, filterMax uint64) {
	if id.tag() != iterTag {
...
func (t *test) setSnapshot(id objID, s *bitalostable.Snapshot) {
	if id.tag() != snapTag {
...
func (t *test) getBatch(id objID) *bitalostable.Batch {
	if id.tag() != batchTag {
		panic(fmt.Sprintf("invalid batch ID: %s", id))
	}
...
func (t *test) getReader(id objID) bitalostable.Reader {
	switch id.tag() {
	case dbTag:
		return t.db
	case batchTag:
...
func (t *test) getWriter(id objID) bitalostable.Writer {
	switch id.tag() {
	case dbTag:
		return t.db
	case batchTag:
