...
import (
	...
	"github.com/zuoyebang/bitalostable/internal/randvar"
...
type iterOpts struct {
	lower    []byte
	upper    []byte
	keyTypes uint32 // bitalostable.IterKeyType
...
func (g *generator) randKeyTypesAndMask() (keyTypes uint32, maskSuffix []byte) {
	...
	case p < 0.2: // 20% probability
		keyTypes = uint32(bitalostable.IterKeyTypePointsOnly)
	case p < 0.8: // 60% probability
		keyTypes = uint32(bitalostable.IterKeyTypePointsAndRanges)
		// With 50% probability, enable masking.
		if g.rng.Intn(2) == 1 {
			maskSuffix = g.randSuffixToRead()
		}
	default: // 20% probability
		keyTypes = uint32(bitalostable.IterKeyTypeRangesOnly)
	}
