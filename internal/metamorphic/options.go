...
func parseOptions(opts *testOptions, data string) error {
	hooks := &bitalostable.ParseHooks{
		NewCache: bitalostable.NewCache,
		NewFilterPolicy: func(name string) (bitalostable.FilterPolicy, error) {
			if name == "none" {
				return nil, nil
			}
...
func defaultOptions() *bitalostable.Options {
	opts := &bitalostable.Options{
		Comparer:           testkeys.Comparer,
		FS:                 vfs.NewMem(),
		FormatMajorVersion: bitalostable.FormatNewest,
		Levels: []bitalostable.LevelOptions{{
...
func randomOptions(rng *rand.Rand) *testOptions {
	...
	// FormatMajorVersion than bitalostable.FormatRangeKeys.
	opts.FormatMajorVersion = bitalostable.FormatRangeKeys
	n := int(bitalostable.FormatNewest - opts.FormatMajorVersion)
	if n > 0 {
		opts.FormatMajorVersion += bitalostable.FormatMajorVersion(rng.Intn(n))
	}
	...
	var lopts bitalostable.LevelOptions
	...
	opts.Levels = []bitalostable.LevelOptions{lopts}
...
var blockPropertyCollectorConstructors = []func() bitalostable.BlockPropertyCollector{
