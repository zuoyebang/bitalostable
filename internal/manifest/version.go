...
func (s CompactionState) String() string {
	...
	case CompactionStateCompacted:
		return "Compacted"
	default:
		panic(fmt.Sprintf("bitalostable: unknown compaction state %d", s))
	}
}
...
type FileMetadata struct {
	...
	Atomic struct {
		...
		// in bitalostable.Iterator after every after every positioning operation
...
func (m *FileMetadata) SetCompactionState(to CompactionState) {
	if invariants.Enabled {
		transitionErr := func() error {
			return errors.Newf("bitalostable: invalid compaction state transition: %s -> %s", m.CompactionState, to)
		}
		...
		case CompactionStateCompacted:
			panic(transitionErr())
		default:
			panic(fmt.Sprintf("bitalostable: unknown compaction state: %d", m.CompactionState))
		}
	}
...
func (l *VersionList) PushBack(v *Version) {
	if v.list != nil || v.prev != nil || v.next != nil {
		panic("bitalostable: version list is inconsistent")
	}
	v.prev = l.root.prev
	v.prev.next = v
	v.next = &l.root
...
func (l *VersionList) Remove(v *Version) {
	if v == &l.root {
		panic("bitalostable: cannot remove version list root node")
	}
	if v.list != l {
		panic("bitalostable: version list is inconsistent")
	}
...
func CheckOrdering(cmp Compare, format base.FormatKey, level Level, files LevelIterator) error {
	...
	if level == Level(0) {
		...
		// with future versions of bitalostable, this method relaxes most L0 invariant
		// checks.
