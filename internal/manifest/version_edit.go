...
var errCorruptManifest = base.CorruptionErrorf("bitalostable: corrupt manifest")
...
func (b *BulkVersionEdit) Accumulate(ve *VersionEdit) error {
	for df, m := range ve.DeletedFiles {
		...
		if m == nil {
			...
			if m == nil {
				return base.CorruptionErrorf("bitalostable: file deleted L%d.%s before it was inserted", df.Level, df.FileNum)
			}
		}
	...
	for _, nf := range ve.NewFiles {
		...
		if dmap := b.Deleted[nf.Level]; dmap != nil {
			if _, ok := dmap[nf.Meta.FileNum]; ok {
				return base.CorruptionErrorf("bitalostable: file deleted L%d.%s before it was inserted", nf.Level, nf.Meta.FileNum)
			}
		}
...
func (b *BulkVersionEdit) Apply(
	...
	if v.Stats.MarkedForCompaction < 0 {
		return nil, nil, base.CorruptionErrorf("bitalostable: version marked for compaction count negative")
	}
	...
	for level := range v.Levels {
			...
			if level == 0 {
				...
				if curr == nil || curr.L0Sublevels == nil {
					if err := v.InitL0Sublevels(cmp, formatKey, flushSplitBytes); err != nil {
						return nil, nil, errors.Wrap(err, "bitalostable: internal error")
					}
				} else {
		...
		if n := v.Levels[level].Len() + len(addedFiles); n == 0 {
			return nil, nil, base.CorruptionErrorf(
				"bitalostable: internal error: No current or added files but have deleted files: %d",
		...
		for _, f := range deletedMap {
			...
			if obsolete := v.Levels[level].tree.delete(f); obsolete {
				...
				err := errors.Errorf("bitalostable: internal error: file L%d.%s obsolete during B-Tree removal", level, f.FileNum)
				return nil, nil, err
			}
			if f.HasRangeKeys {
				if obsolete := v.RangeKeyLevels[level].tree.delete(f); obsolete {
					...
					// previous level's B-Tree, this should never result in a
					// file's reference count dropping to zero.
					err := errors.Errorf("bitalostable: internal error: file L%d.%s obsolete during range-key B-Tree removal", level, f.FileNum)
					return nil, nil, err
				}
			}
		}
		...
		for _, f := range addedFiles {
			...
			if err != nil {
				return nil, nil, errors.Wrap(err, "bitalostable")
			}
			if f.HasRangeKeys {
