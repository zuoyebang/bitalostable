...
package cache // import "github.com/zuoyebang/bitalostable/internal/cache"
...
func (c *shard) Set(id uint64, fileNum base.FileNum, offset uint64, value *Value) Handle {
	if n := value.refs(); n != 1 {
		panic(fmt.Sprintf("bitalostable: Value has already been added to the cache: refs=%d", n))
	}
...
func (c *shard) checkConsistency() {
	...
	case c.sizeHot < 0 || c.sizeCold < 0 || c.sizeTest < 0 || c.countHot < 0 || c.countCold < 0 || c.countTest < 0:
		panic(fmt.Sprintf("bitalostable: unexpected negative: %d (%d bytes) hot, %d (%d bytes) cold, %d (%d bytes) test",
			c.countHot, c.sizeHot, c.countCold, c.sizeCold, c.countTest, c.sizeTest))
	case c.sizeHot > 0 && c.countHot == 0:
		panic(fmt.Sprintf("bitalostable: mismatch %d hot size, %d hot count", c.sizeHot, c.countHot))
	case c.sizeCold > 0 && c.countCold == 0:
		panic(fmt.Sprintf("bitalostable: mismatch %d cold size, %d cold count", c.sizeCold, c.countCold))
	case c.sizeTest > 0 && c.countTest == 0:
		panic(fmt.Sprintf("bitalostable: mismatch %d test size, %d test count", c.sizeTest, c.countTest))
	}
}
	...
	invariants.SetFinalizer(c, func(obj interface{}) {
		...
		if v := atomic.LoadInt64(&c.refs); v != 0 {
			...
			fmt.Fprintf(os.Stderr,
				"bitalostable: cache (%p) has non-zero reference count: %d\n", c, v)
			if len(c.tr.msgs) > 0 {
...
func (c *Cache) getShard(id uint64, fileNum base.FileNum, offset uint64) *shard {
	if id == 0 {
		panic("bitalostable: 0 cache ID is invalid")
	}
...
func (c *Cache) Free(v *Value) {
	if n := v.refs(); n > 1 {
		panic(fmt.Sprintf("bitalostable: Value has been added to the cache: refs=%d", n))
	}
	v.release()
}
...
func (c *Cache) Reserve(n int) func() {
	...
	return func() {
		if shardN == -1 {
			panic("bitalostable: cache reservation already released")
		}
		for i := range c.shards {
			c.shards[i].Reserve(-shardN)
		}
