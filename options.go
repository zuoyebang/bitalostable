...
func parseOptions(s string, fn func(section, key, value string) error) error {
	...
	for _, line := range strings.Split(s, "\n") {
		...
		if pos < 0 {
			return errors.Errorf("bitalostable: invalid key=value syntax: %s", errors.Safe(line))
		}
...
func (o *Options) Parse(s string, hooks *ParseHooks) error {
	return parseOptions(s, func(section, key, value string) error {
		...
		case section == "Version":
			switch key {
			case "bitalostable_version":
			default:
				if hooks != nil && hooks.SkipUnknown != nil && hooks.SkipUnknown(section+"."+key, value) {
					return nil
				}
				return errors.Errorf("bitalostable: unknown option: %s.%s",
		...
		case section == "Options":
			...
			case "l0_sublevel_compactions":
				// Do nothing; option existed in older versions of bitalostable.
			...
			case "min_compaction_rate":
				// Do nothing; option existed in older versions of bitalostable, and
			...
			case "table_format":
				...
				case "rocksdbv2":
				default:
					return errors.Errorf("bitalostable: unknown table format: %q", errors.Safe(value))
				}
		...
		case strings.HasPrefix(section, "Level "):
			...
			if n, err := fmt.Sscanf(section, `Level "%d"`, &index); err != nil {
				...
				return errors.Errorf("bitalostable: unknown section: %q", errors.Safe(section))
			}
			...
			case "compression":
				...
				default:
					return errors.Errorf("bitalostable: unknown compression: %q", errors.Safe(value))
				}
			case "filter_policy":
			...
			case "filter_type":
				...
				default:
					return errors.Errorf("bitalostable: unknown filter type: %q", errors.Safe(value))
				}
			...
			default:
				...
				return errors.Errorf("bitalostable: unknown option: %s.%s", errors.Safe(section), errors.Safe(key))
			}
			return err
		}
		if hooks != nil && hooks.SkipUnknown != nil && hooks.SkipUnknown(section+"."+key, value) {
	...
	return strictWALTail, parseOptions(s, func(section, key, value string) error {
		...
		case "Options.comparer":
			if value != o.Comparer.Name {
				return errors.Errorf("bitalostable: comparer name from file %q != comparer name from options %q",
		...
		case "Options.strict_wal_tail":
			...
			if err != nil {
				return errors.Errorf("bitalostable: error parsing strict_wal_tail value %q: %w", value, err)
			}
		}
		return nil
	})
