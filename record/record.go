...
var (
	...
	ErrNotAnIOSeeker = errors.New("bitalostable/record: reader does not implement io.Seeker")

	// ErrNoLastRecord is returned if LastRecordOffset is called and there is no previous record.
	ErrNoLastRecord = errors.New("bitalostable/record: no last record exists")

	// ErrZeroedChunk is returned if a chunk is encountered that is zeroed. This
	// usually occurs due to log file preallocation.
	ErrZeroedChunk = base.CorruptionErrorf("bitalostable/record: zeroed chunk")
	...
	ErrInvalidChunk = base.CorruptionErrorf("bitalostable/record: invalid chunk")
)
...
func (x singleReader) Read(p []byte) (int, error) {
	r := x.r
	if r.seq != x.seq {
		return 0, errors.New("bitalostable/record: stale reader")
	}
	if r.err != nil {
		return 0, r.err
	}
...
func (w *Writer) fillHeader(last bool) {
	if w.i+legacyHeaderSize > w.j || w.j > blockSize {
		panic("bitalostable/record: bad writer state")
	}
	if last {
		if w.first {
...
func (w *Writer) Close() error {
	...
	w.err = errors.New("bitalostable/record: closed Writer")
	return nil
}
...
func (x singleWriter) Write(p []byte) (int, error) {
	w := x.w
	if w.seq != x.seq {
		return 0, errors.New("bitalostable/record: stale writer")
	}
	if w.err != nil {
		return 0, w.err
	}
