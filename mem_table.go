...
func (m *memTable) apply(batch *Batch, seqNum uint64) error {
	if seqNum < m.logSeqNum {
		return base.CorruptionErrorf("bitalostable: batch seqnum %d is less than memtable creation seqnum %d",
	...
	if seqNum != startSeqNum+uint64(batch.Count()) {
		return base.CorruptionErrorf("bitalostable: inconsistent batch count: %d vs %d",
