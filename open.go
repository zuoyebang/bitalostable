// Copyright 2012 The LevelDB-Go and Pebble and Bitalostored Authors and Bitalostored Authors. All rights reserved. Use
...
import (
	...
	"github.com/zuoyebang/bitalostable/internal/rate"
...
func Open(dirname string, opts *Options) (db *DB, _ error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "bitalostable: database %q", dirname)
		...
		return nil, errors.Errorf("bitalostable: database %q already exists", dirname)
	} else {
	...
	d.opts.Logger.Info("open bitalostable success")
	return d, nil
}
...
func (d *DB) replayWAL(
	...
	for {
		...
		if err != nil {
			...
			return 0, errors.Wrap(err, "bitalostable: error when replaying WAL")
		}

		if buf.Len() < batchHeaderLen {
			return 0, base.CorruptionErrorf("bitalostable: corrupt log file %q (num %s)",
