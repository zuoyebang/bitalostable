...
// permitted usage because it allows (*bitalostable.Options).EnsureDefaults to wrap
// with disk-health checking by default, and to clean up the long-running
// goroutine on (*bitalostable.DB).Close, while still allowing the FS to be used
// multiple times.
