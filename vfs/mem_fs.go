...
package vfs // import "github.com/zuoyebang/bitalostable/vfs"
...
func (y *MemFS) Create(fullname string) (File, error) {
	...
	err := y.walk(fullname, func(dir *memNode, frag string, final bool) error {
		if final {
			if frag == "" {
				return errors.New("bitalostable/vfs: empty file name")
			}
			n := &memNode{name: frag}
			dir.children[frag] = n
			ret = &memFile{
...
func (f *memFile) Close() error {
	if n := atomic.AddInt32(&f.n.refs, -1); n < 0 {
		panic(fmt.Sprintf("bitalostable: close of unopened file: %d", n))
	}
	f.n = nil
	return nil
}
...
func (f *memFile) Read(p []byte) (int, error) {
	if !f.read {
		return 0, errors.New("bitalostable/vfs: file was not opened for reading")
	}
	if f.n.isDir {
		return 0, errors.New("bitalostable/vfs: cannot read a directory")
	}
	f.n.mu.Lock()
	defer f.n.mu.Unlock()
...
func (f *memFile) Write(p []byte) (int, error) {
	if !f.write {
		return 0, errors.New("bitalostable/vfs: file was not created for writing")
	}
	if f.n.isDir {
		return 0, errors.New("bitalostable/vfs: cannot write a directory")
	}
	f.n.mu.Lock()
	defer f.n.mu.Unlock()
