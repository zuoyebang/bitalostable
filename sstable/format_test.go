...
func TestTableFormat_RoundTrip(t *testing.T) {
	tcs := []struct {
		...
		// Invalid cases.
		{
			...
			magic:   rocksDBMagic,
			version: 1,
			wantErr: "bitalostable/table: unsupported rocksdb format version 1",
		},
		{
			name:    "Invalid PebbleDB version",
			magic:   bitalostableDBMagic,
			version: 3,
			wantErr: "bitalostable/table: unsupported bitalostable format version 3",
		},
		{
			name:    "Unknown magic string",
			magic:   "foo",
			wantErr: "bitalostable/table: invalid table (bad magic number)",
		},
	}
