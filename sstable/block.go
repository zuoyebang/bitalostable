...
func (i *blockIter) init(cmp Compare, block block, globalSeqNum uint64) error {
	...
	if numRestarts == 0 {
		return base.CorruptionErrorf("bitalostable/table: invalid table (block has no restart points)")
	}
	i.cmp = cmp
...
func (i *blockIter) readFirstKey() error {
	...
	if n := len(firstKey) - 8; n >= 0 {
		...
		return base.CorruptionErrorf("bitalostable/table: invalid firstKey in block")
	}
	return nil
}
...
// Last implements internalIterator.Last, as documented in the bitalostable package.
...
// Key implements internalIterator.Key, as documented in the bitalostable package.
func (i *blockIter) Key() *InternalKey {
	return &i.ikey
}

// Value implements internalIterator.Value, as documented in the bitalostable
// package.
...
func (i *blockIter) SetBounds(lower, upper []byte) {
	...
	panic("bitalostable: SetBounds unimplemented")
}
...
func (i *fragmentBlockIter) decodeSpanKeys(k *InternalKey, internalValue []byte) {
	...
	default:
		i.span = keyspan.Span{}
		i.err = base.CorruptionErrorf("bitalostable: corrupt keyspan fragment of kind %d", k.Kind())
	}
}
...
func (i *fragmentBlockIter) gatherForward(k *InternalKey, internalValue []byte) *keyspan.Span {
	...
	for k != nil && i.blockIter.cmp(k.UserKey, i.span.Start) == 0 {
		...
		if i.blockIter.cmp(prevEnd, i.span.End) != 0 {
			i.err = base.CorruptionErrorf("bitalostable: corrupt keyspan fragmentation")
			i.span = keyspan.Span{}
			return nil
		}
		k, internalValue = i.blockIter.Next()
...
func (i *fragmentBlockIter) Next() *keyspan.Span {
	...
	case i.dir == -1 && i.span.Valid():
		...
		if x := i.gatherForward(i.blockIter.Next()); invariants.Enabled && !x.Valid() {
			panic("bitalostable: invariant violation: next entry unexpectedly invalid")
		}
		i.dir = +1
	}
...
func (i *fragmentBlockIter) Prev() *keyspan.Span {
	...
	case i.dir == +1 && i.span.Valid():
		...
		if x := i.gatherBackward(i.blockIter.Prev()); invariants.Enabled && !x.Valid() {
			panic("bitalostable: invariant violation: previous entry unexpectedly invalid")
		}
		i.dir = -1
	}
