...
import (
	...
	"github.com/zuoyebang/bitalostable/internal/crc"
	"github.com/zuoyebang/bitalostable/internal/invariants"
	"github.com/zuoyebang/bitalostable/internal/keyspan"
	"github.com/zuoyebang/bitalostable/internal/private"
	"github.com/zuoyebang/bitalostable/vfs"
)

var errCorruptIndexEntry = base.CorruptionErrorf("bitalostable/table: corrupt index entry")
var errReaderClosed = errors.New("bitalostable/table: reader is closed")
...
// SeekGE implements internalIterator.SeekGE, as documented in the bitalostable
...
// bitalostable package. Note that SeekPrefixGE only checks the upper bound. It is up
...
// SeekLT implements internalIterator.SeekLT, as documented in the bitalostable
...
// First implements internalIterator.First, as documented in the bitalostable
...
// Last implements internalIterator.Last, as documented in the bitalostable
...
// Next implements internalIterator.Next, as documented in the bitalostable
// package.
...
// Prev implements internalIterator.Prev, as documented in the bitalostable
// package.
...
// Error implements internalIterator.Error, as documented in the bitalostable
// package.
...
// Close implements internalIterator.Close, as documented in the bitalostable
// package.
...
// SetBounds implements internalIterator.SetBounds, as documented in the bitalostable
// package.
...
func (i *compactionIterator) SeekGE(key []byte, flags base.SeekGEFlags) (*InternalKey, []byte) {
	panic("bitalostable: SeekGE unimplemented")
}

func (i *compactionIterator) SeekPrefixGE(
	prefix, key []byte, flags base.SeekGEFlags,
) (*base.InternalKey, []byte) {
	panic("bitalostable: SeekPrefixGE unimplemented")
}

func (i *compactionIterator) SeekLT(key []byte, flags base.SeekLTFlags) (*InternalKey, []byte) {
	panic("bitalostable: SeekLT unimplemented")
}
...
func (i *compactionIterator) Last() (*InternalKey, []byte) {
	panic("bitalostable: Last unimplemented")
}
...
func (i *compactionIterator) Prev() (*InternalKey, []byte) {
	panic("bitalostable: Prev unimplemented")
}
...
func (i *twoLevelIterator) loadIndex(dir int8) loadBlockResult {
	...
	if err != nil {
		i.err = base.CorruptionErrorf("bitalostable/table: corrupt top level index entry")
		return loadBlockFailed
	}
	if i.bpfs != nil {
		intersects, err := i.bpfs.intersects(bhp.Props)
...
// Close implements DB.Close, as documented in the bitalostable package.
...
func (r *Reader) NewIterWithBlockPropertyFilters(
	...
	// NB: bitalostable.tableCache wraps the returned iterator with one which performs
...
func checkChecksum(
	...
	if expectedChecksum != computedChecksum {
		return base.CorruptionErrorf(
			"bitalostable/table: invalid table %s (checksum mismatch at %d/%d)",
...
func (r *Reader) readMetaindex(metaindexBH BlockHandle) error {
	...
	if uint64(len(data)) != metaindexBH.Length {
		return base.CorruptionErrorf("bitalostable/table: unexpected metaindex block size: %d vs %d",
	...
	for valid := i.First(); valid; valid = i.Next() {
		...
		if n == 0 {
			return base.CorruptionErrorf("bitalostable/table: invalid table (bad filter block handle)")
		}
...
func NewReader(f ReadableFile, o ReaderOptions, extraOpts ...ReaderOption) (*Reader, error) {
	...
	if f == nil {
		r.err = errors.New("bitalostable/table: nil file")
		return nil, r.Close()
	}
	...
	if r.Compare == nil {
		r.err = errors.Errorf("bitalostable/table: %d: unknown comparer %s",
			errors.Safe(r.fileNum), errors.Safe(r.Properties.ComparerName))
	}
	if !r.mergerOK {
		if name := r.Properties.MergerName; name != "" && name != "nullptr" {
			r.err = errors.Errorf("bitalostable/table: %d: unknown merger %s",
