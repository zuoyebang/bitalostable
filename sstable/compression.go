...
func decompressedLen(blockType blockType, b []byte) (int, int, error) {
	...
	case zstdCompressionBlockType:
		...
		if varIntLen <= 0 {
			return 0, 0, base.CorruptionErrorf("bitalostable/table: compression block has invalid length")
		}
		return int(decodedLenU64), varIntLen, nil
	default:
		return 0, 0, base.CorruptionErrorf("bitalostable/table: unknown block compression: %d", errors.Safe(blockType))
	}
}
...
func decompressInto(blockType blockType, compressed []byte, buf []byte) ([]byte, error) {
	...
	if len(result) != 0 && (len(result) != len(buf) || &result[0] != &buf[0]) {
		return nil, base.CorruptionErrorf("bitalostable/table: decompressed into unexpected buffer: %p != %p",
