...
// Package sstable implements readers and writers of bitalostable tables.
//
...
//	}
//	return w.Close()
package sstable // import "github.com/zuoyebang/bitalostable/sstable"
...
independently. The checksum algorithm is described in the bitalostable/crc package.
...
const (
	...
	bitalostableDBMagic = "\xf0\x9f\xaa\xb3\xf0\x9f\xaa\xb3" // 🪳🪳
	...
	metaRangeKeyName   = "bitalostable.range_key"
...
func readFooter(f ReadableFile) (footer, error) {
	...
	stat, err := f.Stat()
	if err != nil {
		return footer, errors.Wrap(err, "bitalostable/table: invalid table (could not stat file)")
	}
	if stat.Size() < minFooterLen {
		return footer, base.CorruptionErrorf("bitalostable/table: invalid table (file size is too small)")
	}
	...
	if err != nil && err != io.EOF {
		return footer, errors.Wrap(err, "bitalostable/table: invalid table (could not read footer)")
	}
	buf = buf[:n]
	...
	case levelDBMagic:
		if len(buf) < levelDBFooterLen {
			return footer, base.CorruptionErrorf(
				"bitalostable/table: invalid table (footer too short): %d", errors.Safe(len(buf)))
		}
	...
	case rocksDBMagic, bitalostableDBMagic:
		...
		if len(buf) < rocksDBFooterLen {
			return footer, base.CorruptionErrorf("bitalostable/table: invalid table (footer too short): %d", errors.Safe(len(buf)))
		}
		...
		default:
			return footer, base.CorruptionErrorf("bitalostable/table: unsupported checksum type %d", errors.Safe(footer.checksum))
		}
		buf = buf[1:]

	default:
		return footer, base.CorruptionErrorf("bitalostable/table: invalid table (bad magic number)")
	}

	{
		...
		if n == 0 || footer.metaindexBH.Offset+footer.metaindexBH.Length > end {
			return footer, base.CorruptionErrorf("bitalostable/table: invalid table (bad metaindex block handle)")
		}
		buf = buf[n:]

		footer.indexBH, n = decodeBlockHandle(buf)
		if n == 0 || footer.indexBH.Offset+footer.indexBH.Length > end {
			return footer, base.CorruptionErrorf("bitalostable/table: invalid table (bad index block handle)")
		}
	}
