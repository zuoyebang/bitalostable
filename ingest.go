// Copyright 2018 The LevelDB-Go and Pebble Authors and Bitalostored Authors. All rights reserved. Use
...
func ingestValidateKey(opts *Options, key *InternalKey) error {
	if key.Kind() == InternalKeyKindInvalid {
		return base.CorruptionErrorf("bitalostable: external sstable has corrupted key: %s",
			key.Pretty(opts.Comparer.FormatKey))
	}
	if key.SeqNum() != 0 {
		return base.CorruptionErrorf("bitalostable: external sstable has non-zero seqnum: %s",
			key.Pretty(opts.Comparer.FormatKey))
	}
	return nil
}
...
func ingestLoad1(
	...
	if tf < fmv.MinTableFormat() || tf > fmv.MaxTableFormat() {
		return nil, errors.Newf(
			"bitalostable: table format %s is not within range supported at DB format major version %d, (%s,%s)",
...
func ingestSortAndVerify(cmp Compare, meta []*fileMetadata, paths []string) error {
	...
	for i := 1; i < len(meta); i++ {
		if sstableKeyCompare(cmp, meta[i-1].Largest, meta[i].Smallest) >= 0 {
			return errors.New("bitalostable: external sstables have overlapping ranges")
		}
	}
	return nil
}
...
func ingestLink(
	...
	for i := range paths {
		...
		if _, ok := opts.FS.(*vfs.MemFS); ok && opts.DebugCheck != nil {
			...
			// occurs. See https://github.com/zuoyebang/bitalostable/issues/495.
...
// https://github.com/zuoyebang/bitalostable/pull/835#issuecomment-663075379
//
...
// https://github.com/zuoyebang/bitalostable/issues/25 for an idea for how to fix
// this hiccup.
...
type IngestOperationStats struct {
	...
	// be approximate once https://github.com/zuoyebang/bitalostable/issues/25 is
	// implemented.
...
func (d *DB) validateSSTables() {
	...
	for _, f := range pending {
		...
		if err != nil {
			...
			// available. See bitalostable#1192.
			d.opts.Logger.Fatalf("bitalostable: encountered corruption during ingestion: %s", err)
		}
