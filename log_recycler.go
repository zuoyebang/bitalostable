...
func (r *logRecycler) pop(logNum FileNum) error {
	...
	if len(r.mu.logs) == 0 {
		return errors.New("bitalostable: log recycler empty")
	}
	if r.mu.logs[0].fileNum != logNum {
		return errors.Errorf("bitalostable: log recycler invalid %d vs %d", errors.Safe(logNum), errors.Safe(fileInfoNums(r.mu.logs)))
	}
