// Copyright 2012 The LevelDB-Go and Pebble Authors and Bitalostored Authors. All rights reserved. Use
...
import (
	...
	"github.com/zuoyebang/bitalostable/internal/arenaskl"
...
var (
	...
	ErrClosed = errors.New("bitalostable: closed")
	// ErrReadOnly is returned when a write operation is performed on a read-only
	// database.
	ErrReadOnly = errors.New("bitalostable: read-only")
	...
	// implementation.
	errNoSplit = errors.New("bitalostable: Comparer.Split required for range key operations")
)
...
func (d *DB) Apply(batch *Batch, opts *WriteOptions) error {
	...
	if atomic.LoadUint32(&batch.applied) != 0 {
		panic("bitalostable: batch already applied")
	}
	if d.opts.ReadOnly {
		return ErrReadOnly
	}
	if batch.db != nil && batch.db != d {
		panic(fmt.Sprintf("bitalostable: batch db mismatch: %p != %p", batch.db, d))
	}

	sync := opts.GetSync()
	if sync && d.opts.DisableWAL {
		return errors.New("bitalostable: WAL disabled")
	}
	...
	if batch.countRangeKeys > 0 {
		...
		if d.FormatMajorVersion() < FormatRangeKeys {
			panic(fmt.Sprintf(
				"bitalostable: range keys require at least format major version %d (current: %d)",
	...
	if int(batch.memTableSize) >= d.largeBatchThreshold || batch.isFlush {
...
func (d *DB) commitWrite(b *Batch, syncWG *sync.WaitGroup, syncErr *error) (*memTable, error) {
	...
	if b.flushable != nil {
		...
		if !d.opts.DisableWAL && !b.isFlush {
			var err error
...
func (d *DB) newIterInternal(batch *Batch, s *Snapshot, o *IterOptions) *Iterator {
	...
	if o != nil && o.RangeKeyMasking.Suffix != nil && o.KeyTypes != IterKeyTypePointsAndRanges {
		panic("bitalostable: range key masking requires IterKeyTypePointsAndRanges")
	}
...
func (d *DB) NewFlushBatch(n int) *Batch {
	return newFlushBatch(d, n)
}
...
func (d *DB) Close() error {
	...
	if n := len(d.mu.compact.inProgress); n > 0 {
		err = errors.Errorf("bitalostable: %d unexpected in-progress compactions", errors.Safe(n))
	}
	...
	if !d.opts.ReadOnly {
		...
		panic("bitalostable: log-writer should be nil in read-only mode")
	}
...
func (d *DB) SetOptsDisableAutomaticCompactions(v bool) {
	if d.opts.DisableAutomaticCompactions != v {
			...
			if ingestMemtableOverlaps(d.cmp, mem, meta) {
				...
				if mem.flushable == d.mu.mem.mutable {
					...
					if mem.flushable == d.mu.mem.mutable {
						...
						err = d.makeRoomForWrite(nil, true)
					}
				}
...
func (d *DB) makeRoomForWrite(b *Batch, needReport bool) error {
	...
	isFlushBatch := b != nil && b.flushable != nil && b.isFlush
	stalled := false
	for {
