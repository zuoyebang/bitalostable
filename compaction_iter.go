...
import (
	...
	"github.com/zuoyebang/bitalostable/internal/bytealloc"
...
func (i *compactionIter) nextInStripe() stripeChangeType {
	...
	if i.keyTrailer <= base.InternalKeyZeroSeqnumMaxTrailer || key.Trailer >= i.keyTrailer {
		if invariants.Enabled && i.equal(i.key.UserKey, key.UserKey) {
			...
			panic(fmt.Sprintf("bitalostable: invariant violation: %s and %s out of order", key, prevKey))
		}
...
func (i *compactionIter) setNext() {
	...
	for {
		...
		default:
			panic("bitalostable: unexpected stripeChangeType: " + strconv.Itoa(int(t)))
		}
	}
}
