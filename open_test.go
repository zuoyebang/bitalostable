...
func TestTwoWALReplayCorrupt(t *testing.T) {
	...
	_, err = Open(dir, nil)
	require.Error(t, err, "bitalostable: corruption")
}
...
func TestRocksDBNoFlushManifest(t *testing.T) {
	...
	// a next-log-num but no log-num entry. Ensure that bitalostable can read these
...
func TestOpen_ErrorIfUnknownFormatVersion(t *testing.T) {
	...
	require.Error(t, err)
	require.EqualError(t, err, `bitalostable: database "" written in format major version 999999`)
}
