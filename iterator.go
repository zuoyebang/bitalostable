...
import (
	...
	"github.com/zuoyebang/bitalostable/internal/fastrand"
...
// https://github.com/zuoyebang/bitalostable/issues/29#issuecomment-494477985
const readBytesPeriod uint64 = 1 << 16

var errReversePrefixIteration = errors.New("bitalostable: unsupported reverse prefix iteration")
...
func (i *Iterator) findNextEntry(limit []byte) {
	...
	for i.iterKey != nil {
		...
		default:
			i.err = base.CorruptionErrorf("bitalostable: invalid internal key kind: %d", errors.Safe(key.Kind()))
			i.iterValidityState = IterExhausted
			return
		}
	}
...
func (i *Iterator) nextPointCurrentUserKey() bool {
	...
	case InternalKeyKindRangeKeySet:
		...
		// for a user key.
		i.err = base.CorruptionErrorf("bitalostable: unexpected range key set mid-user key")
		return false
		...
		if len(mi.levels) > 1 {
			mi.ForEachLevelIter(func(li *levelIter) bool {
				...
				if file := li.files.Current(); file != nil {
					...
					// at:
					// https://github.com/zuoyebang/bitalostable/pull/1041#issuecomment-763226492
					if containsKey {
...
func (i *Iterator) mergeNext(key InternalKey, valueMerger ValueMerger) {
	...
	for {
		...
		case InternalKeyKindRangeKeySet:
			...
			i.err = base.CorruptionErrorf("bitalostable: out of order range key marker")
			return
...
//
//	iter := db.NewIter(&bitalostable.IterOptions{
...
func (i *Iterator) SeekPrefixGE(key []byte) bool {
	...
	if i.comparer.Split == nil {
		panic("bitalostable: split must be provided for SeekPrefixGE")
	}
	if i.comparer.ImmediateSuccessor == nil && i.opts.KeyTypes != IterKeyTypePointsOnly {
		panic("bitalostable: ImmediateSuccessor must be provided for SeekPrefixGE with range keys")
	}
	...
	if lowerBound := i.opts.GetLowerBound(); lowerBound != nil && i.cmp(key, lowerBound) < 0 {
		if n := i.split(lowerBound); !bytes.Equal(i.prefixOrFullSeekKey, lowerBound[:n]) {
			i.err = errors.New("bitalostable: SeekPrefixGE supplied with key outside of lower bound")
			i.iterValidityState = IterExhausted
			return false
		}
		key = lowerBound
		...
		if n := i.split(upperBound); !bytes.Equal(i.prefixOrFullSeekKey, upperBound[:n]) {
			i.err = errors.New("bitalostable: SeekPrefixGE supplied with key outside of upper bound")
			i.iterValidityState = IterExhausted
			return false
		}
		key = upperBound
...
func (i *Iterator) saveRangeKey() {
	...
	if s.KeysOrder != keyspan.BySuffixAsc {
		panic("bitalostable: range key span's keys unexpectedly not in ascending suffix order")
	}
	...
	for j := 0; j < len(s.Keys); j++ {
		if invariants.Enabled {
			if s.Keys[j].Kind() != base.InternalKeyKindRangeKeySet {
				panic("bitalostable: user iteration encountered non-RangeKeySet key kind")
			} else if j > 0 && i.cmp(s.Keys[j].Suffix, s.Keys[j-1].Suffix) < 0 {
				panic("bitalostable: user iteration encountered range keys not in suffix order")
			}
		}
