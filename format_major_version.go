...
func (v FormatMajorVersion) MaxTableFormat() sstable.TableFormat {
	...
	default:
		panic(fmt.Sprintf("bitalostable: unsupported format major version: %s", v))
	}
}
...
func lookupFormatMajorVersion(
	...
	if vers == FormatDefault {
		return 0, nil, errors.Newf("bitalostable: default format major version should not persisted", vers)
	}
	if vers > FormatNewest {
		return 0, nil, errors.Newf("bitalostable: database %q written in format major version %d", dirname, vers)
	}
	return vers, m, nil
}
...
func (d *DB) ratchetFormatMajorVersionLocked(formatVers FormatMajorVersion) error {
	...
	if formatVers > FormatNewest {
		...
		return errors.Errorf("bitalostable: unknown format version %d", formatVers)
	}
	if d.mu.formatVers.vers > formatVers {
		return errors.Newf("bitalostable: database already at format major version %d; cannot reduce to %d",
			d.mu.formatVers.vers, formatVers)
	}
	if d.mu.formatVers.ratcheting {
		return errors.Newf("bitalostable: database format major version upgrade is in-progress")
	}
	...
	for nextVers := d.mu.formatVers.vers + 1; nextVers <= formatVers; nextVers++ {
		...
		if d.mu.formatVers.vers != nextVers {
			d.opts.Logger.Fatalf("bitalostable: successful migration to format version %d never finalized the upgrade", nextVers)
		}
	}
	return nil
}
